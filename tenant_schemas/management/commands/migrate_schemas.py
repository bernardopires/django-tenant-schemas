...
class Command(SyncCommon):
    ...
    def _set_managed_apps(self, included_apps, excluded_apps):
        """ Set which apps should be migrated.

        While sync_schemas works by setting which apps are managed,
        on south we set which apps should be ignored.
        """
        ignored_apps = []
        if excluded_apps:
    ...
    def _save_south_settings(self):
        ...
        if (hasattr(settings, "SOUTH_MIGRATION_MODULES")
                and settings.SOUTH_MIGRATION_MODULES is not None):
    ...
    def migrate_tenant_apps(self, schema_name=None):
        ...
        if schema_name:
            ...
            for tenant in all_tenants:
                # very important, the dependencies need to be purged from cache
